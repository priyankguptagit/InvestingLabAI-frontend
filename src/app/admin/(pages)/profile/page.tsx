  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      <div
        ...
        onDrop={handleDrop}
        onClick={() => !uploading && inputRef.current?.click()}
        className={`w-full max-w-xs flex flex-col items-center gap-2 py-5 px-4 rounded-2xl border-2 border-dashed cursor-pointer transition-all duration-200 ${
          dragging
      ...
      >
        <div
          className={`h-9 w-9 rounded-full flex items-center justify-center transition-colors ${dragging ? "bg-blue-500/20 text-blue-400" : "bg-slate-700/60 text-slate-400"}`}
        >
          <Upload className="h-4 w-4" />
        </div>
        <div className="text-center">
          <p
            className={`text-xs font-semibold ${dragging ? "text-blue-300" : "text-slate-300"}`}
          >
...
function ChangePasswordModal({
  ...
  const displayError =
    localError || (alert?.type === "error" ? alert.message : null);
          ...
          <div className="space-y-1.5">
            ...
            {newPw.length > 0 && (
              <div className="space-y-1.5 pt-1">
                ...
                <p
                  className={`text-[11px] font-semibold ${pwStrengthTextColor[pwStrength]}`}
                >
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-9 pr-10 h-11 bg-slate-800/70 border rounded-xl text-white text-sm placeholder:text-slate-600 focus:outline-none focus:ring-1 transition-all ${
...
export default function AdminProfilePage() {
  ...
  const handleManualCleanup = async () => {
    ...
    try {
      ...
      setCleanupError(
        err?.response?.data?.message || "Cleanup failed. Please try again.",
      );
    } finally {
  ...
  const handleAvatarSelect = async (file: File) => {
    ...
    try {
      ...
      const res = await axiosInstance.post(
        "/api/upload?folder=praedico_admin_avatars",
        formData,
        {
    ...
    if (phone.trim()) {
      ...
      const isValid =
        ...
        (cleanPhone.length === 12 &&
          ...
          /^[6-9]/.test(cleanPhone.substring(2)));
      if (!isValid) {
        setProfileAlert({
          type: "error",
      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="relative rounded-3xl overflow-hidden mb-8 h-48">
          ...
          <div className="absolute top-6 right-8">
            <div
              className={`px-3 py-1.5 rounded-full border text-xs font-bold tracking-wide ${roleBadgeClass(profile?.role || "")}`}
            >
          ...
          {[
            {
              ...
              icon: Fingerprint,
              color: profile?.isActive ? "text-emerald-400" : "text-red-400",
              bg: profile?.isActive ? "bg-emerald-500/10" : "bg-red-500/10",
            },
            {
              label: "Role",
            ...
            <div
              key={label}
              className="bg-slate-900/60 border border-slate-800/60 rounded-2xl p-4 backdrop-blur-sm"
            >
              ...
              <Card className="bg-slate-900/60 border-slate-800/60 backdrop-blur-sm rounded-2xl shadow-xl">
                <CardHeader className="pb-2">
                  ...
                  <CardDescription className="text-slate-400 text-xs">
                    Drag & drop or click the avatar to upload. Stored
                    independently from your user account.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <AvatarUploadZone
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        className={`pl-9 bg-slate-800/60 border-slate-700 text-white placeholder:text-slate-500 focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 rounded-xl h-11 transition-all ${
                          phone &&
                          !(
                            (phone.replace(/\D/g, "").length === 10 &&
                              /^[6-9]/.test(phone.replace(/\D/g, ""))) ||
                            (phone.replace(/\D/g, "").length === 12 &&
                              phone.replace(/\D/g, "").startsWith("91") &&
                              /^[6-9]/.test(
                    ...
                    {phone &&
                      !(
                        ...
                        (phone.replace(/\D/g, "").length === 12 &&
                          phone.replace(/\D/g, "").startsWith("91") &&
                          /^[6-9]/.test(phone.replace(/\D/g, "").substring(2)))
                      ) && (
                        <p className="text-[11px] text-red-400 font-medium flex items-center gap-1 mt-1">
                          <AlertCircle className="h-3 w-3" /> Please enter a
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-3 px-3 h-11 bg-slate-800/30 border border-slate-700/50 rounded-xl">
                      ...
                      {profile?.customRole && (
                        <Badge
                          variant="outline"
                          className="ml-auto text-[11px] border-slate-600 text-slate-400"
                        >
                    ...
                    <p className="text-[11px] text-slate-600">
                      Roles are managed by your super admin.
                    </p>
                  </div>
                  ...
                  <CardHeader className="pb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <CardDescription className="text-slate-400 text-xs">
                          Compact past-day intraday records into DailySnapshots
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex items-start gap-3 p-3.5 rounded-xl bg-slate-800/40 border border-slate-700/40">
                      ...
                      <p className="text-slate-400 text-xs leading-relaxed">
                        Runs the same cleanup that automatically fires every
                        trading day morning. Safe to run at any time —
                        today&apos;s intraday data is never touched. Past-day
                        records are compacted into sealed DailySnapshots first.
                      </p>
                    </div>
                      ...
                      <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/25 text-emerald-300 text-sm animate-in fade-in duration-300">
                        ...
                        <div>
                          ...
                          <p className="text-emerald-400/70 text-xs mt-0.5">
                            {cleanupResult.deletedCount} records deleted &mdash;{" "}
                            {cleanupResult.remainingPastIntraday} past-day
                            records remaining
                          </p>
                        </div>
                      </div>
