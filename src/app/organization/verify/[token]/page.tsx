...
import {
  Loader2, Eye, EyeOff, Lock, CheckCircle2, ShieldCheck,
  CheckCircle, AlertCircle, X, Check, Building2
} from "lucide-react";
import { z } from "zod";
...
export default function OrganizationVerifyPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const handleVerification = async (e: React.FormEvent) => {
    ...
    try {
      await organizationApi.verify({ token, password });
      setIsSuccess(true);
      setTimeout(() => {
        router.push("/organization/dashboard");
      }, 2500);
    } catch (err: any) {
      setError(err.response?.data?.message || "Link expired or invalid. Please contact support.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-slate-50 dark:bg-[#0B1120] p-4 font-sans selection:bg-indigo-500/30">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-[-10%] right-[-10%] w-[600px] h-[600px] rounded-full bg-violet-500/20 dark:bg-violet-600/20 blur-[120px] mix-blend-multiply dark:mix-blend-screen animate-pulse delay-1000" />
      </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] shadow-2xl shadow-indigo-500/10 p-8 sm:p-10">
          ...
          {isSuccess ? (
            <div className="flex flex-col items-center justify-center text-center py-10 space-y-6 animate-in fade-in zoom-in duration-500">
              ...
              <div>
                <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Organization Verified!</h2>
                <p className="text-slate-500 mt-2 font-medium">Your account is now active.<br />Redirecting to your dashboard...</p>
              </div>
            ...
            <div>
              <div className="mb-8 text-center">
                ...
                <h1 className="text-3xl font-black text-slate-900 dark:text-white mb-3 tracking-tight">Verify Organization</h1>
                <p className="text-slate-500 font-medium text-sm px-4">Set a strong admin password to activate your organization account.</p>
              </div>
              ...
              <form onSubmit={handleVerification} className="space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-500 ml-1">Admin Password</label>
                ...
                >
                  {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Activate Organization Account"}
                </button>
