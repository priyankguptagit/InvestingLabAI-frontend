    ...
    <div className="rounded-2xl bg-[#111827] border border-white/5 overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300">
      <div className="px-6 pt-5 pb-4 border-b border-white/5">
        <div className="flex items-center gap-3">
          <div
            className={`h-8 w-8 rounded-xl ${bgMap[accentColor]} border flex items-center justify-center`}
          >
...
function StatTile({
  ...
  return (
    <div
      className={`rounded-xl border ${colorMap[color]} p-4 flex items-center gap-3 group hover:scale-[1.02] transition-transform duration-200`}
    >
      <div
        className={`h-10 w-10 rounded-xl ${colorMap[color]} flex items-center justify-center flex-shrink-0`}
      >
...
function SubscriptionBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border uppercase tracking-wider ${styles[status] || styles.inactive}`}
    >
      <span
        className={`h-1.5 w-1.5 rounded-full ${status === "active" ? "bg-emerald-400 animate-pulse" : status === "expired" ? "bg-rose-400" : "bg-slate-500"}`}
      />
      {status || "Unknown"}
    </span>
  );
...
export default function OrganizationSettingsPage() {
  ...
  const inputClass =
    "w-full px-4 py-3 bg-[#0f172a] border border-slate-700/50 rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/30 transition-all";
  ...
  return (
    <div className="min-h-screen w-full bg-[#030712] text-slate-200 font-sans selection:bg-indigo-500/30 relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none">
        <div
          className="absolute top-[-15%] right-[-5%] w-[500px] h-[500px] bg-blue-500/8 rounded-full blur-[120px] animate-pulse"
          style={{ animationDuration: "8s" }}
        />
        <div
          className="absolute bottom-[-10%] left-[-5%] w-[400px] h-[400px] bg-indigo-500/6 rounded-full blur-[100px] animate-pulse"
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 py-8 pb-20">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/10 text-blue-300 text-[11px] font-bold uppercase tracking-wider border border-white/10 backdrop-blur-md">
                  ...
                  {ORG_TYPES.find((t) => t.value === form.organizationType)
                    ?.label || form.organizationType}
                </span>
                <StatusBadge
                  active={readOnly.isVerified}
                  label={readOnly.isVerified ? "Verified" : "Unverified"}
                />
                <StatusBadge
                  active={readOnly.isActive}
                  label={readOnly.isActive ? "Active" : "Inactive"}
                />
        ...
        {status !== "idle" && (
          <div
            className={`mb-6 flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-medium animate-fade-in
                        ${
                          status === "success"
        ...
        <div className="space-y-6">
          ...
          <SectionCard
            ...
            subtitle="Managed by the Praedico platform team"
            accentColor="amber"
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="rounded-xl bg-[#0a0f1a] border border-slate-800/50 p-4">
                ...
                <p className="text-base font-bold text-white">
                  {readOnly.subscriptionPlan || "No Plan"}
                </p>
              </div>
              <div className="rounded-xl bg-[#0a0f1a] border border-slate-800/50 p-4">
                <p className="text-[11px] text-slate-500 font-medium mb-1">
              ...
              <div className="rounded-xl bg-[#0a0f1a] border border-slate-800/50 p-4">
                ...
                <p className="text-base font-bold text-white">
                  {readOnly.subscriptionExpiry
                    ? new Date(readOnly.subscriptionExpiry).toLocaleDateString(
                        "en-US",
          ...
          <SectionCard
            ...
            subtitle="Live metrics across your organization"
            accentColor="emerald"
          >
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
              <StatTile
                icon={Building2}
                label="Departments"
                value={readOnly.totalDepartments}
                color="indigo"
              />
              <StatTile
                icon={ShieldCheck}
                label="Admins"
                value={readOnly.totalAdmins}
                color="blue"
              />
              <StatTile
                icon={Users}
                label="Coordinators"
                value={readOnly.totalCoordinators}
                color="purple"
              />
              <StatTile
                icon={GraduationCap}
                label="Total Students"
                value={readOnly.totalStudents}
                color="emerald"
              />
              <StatTile
                icon={BadgeCheck}
                label="Active Students"
                value={readOnly.activeStudents}
                color="amber"
              />
              <StatTile
                icon={UserCheck}
                label="Pending Approvals"
                value={readOnly.pendingApprovals}
                color="rose"
              />
            </div>
          </SectionCard>
          ...
          {readOnly.registeredBy && (
            <SectionCard
              icon={ShieldCheck}
              title="Registered By"
              subtitle="Initial registrant details"
              accentColor="purple"
            >
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <ReadOnlyField
                  icon={Users}
                  label="Name"
                  value={readOnly.registeredBy.name}
                />
                <ReadOnlyField
                  icon={Mail}
                  label="Email"
                  value={readOnly.registeredBy.email}
                />
                <ReadOnlyField
                  icon={BadgeCheck}
                  label="Designation"
                  value={readOnly.registeredBy.designation}
                />
              </div>
            </SectionCard>
          )}
