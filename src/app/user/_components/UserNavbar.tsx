    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 w-full transition-all duration-500 animate-slide-down ${
          scrolled || mobileMenuOpen || isPremiumPage
                ...
                const badgeContent = (
                  <div className="hidden md:flex items-center gap-2.5 px-3 py-1.5 rounded-full bg-white/5 dark:bg-slate-800/50 border border-white/10 dark:border-slate-700/40 backdrop-blur-md shadow-sm">
                    ...
                    {orgLogoUrl && !orgLogoError ? (
                      ...
                      <span className="text-xs font-semibold text-slate-400 dark:text-slate-300 tracking-wide max-w-[120px] truncate">
                        {orgName}
                      </span>
                    )}
                    {orgLogoUrl && !orgLogoError && orgName && (
                      <span className="text-[11px] font-medium text-slate-500 dark:text-slate-400 hidden lg:block truncate max-w-[100px]">
                        {orgName}
                      </span>
                    )}
                  </div>
              ...
              {visibleNavItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`relative flex items-center gap-1.5 xl:gap-2 px-3 xl:px-4 2xl:px-5 py-2 rounded-full text-xs xl:text-sm font-semibold transition-all duration-300 ${
                      isActive
                  ...
                  >
                    ...
                    <Icon
                      size={16}
                      className={`${isActive ? "stroke-[2.5px]" : "stroke-[2px] opacity-70"} xl:w-[18px] xl:h-[18px]`}
                    />
                    {item.label}
                  </Link>
                );
                ...
                <DropdownMenuTrigger asChild>
                  <button className="flex items-center gap-2 xl:gap-2.5 pl-1.5 pr-2 xl:pr-3 py-1.5 rounded-full bg-white/80 dark:bg-slate-900/80 border border-slate-200/60 dark:border-slate-700/60 shadow-sm hover:shadow-md hover:border-indigo-100 transition-all duration-300 group backdrop-blur-sm max-w-[160px] xl:max-w-[220px]">
                    ...
                    <div className="text-left hidden 2xl:block min-w-0 flex-1">
                      <p className="text-xs font-bold text-slate-700 dark:text-slate-200 group-hover:text-indigo-600 transition-colors truncate max-w-[90px] xl:max-w-[110px]">
                        {userName}
                      </p>
                      <p className="text-[10px] font-medium text-slate-400 uppercase tracking-wider truncate max-w-[90px] xl:max-w-[110px]">
                        {currentPlan} Account
                      </p>
                    </div>
                    <ChevronDown
