...
import { Loader2, Eye, EyeOff, Lock, CheckCircle2, ShieldCheck, CheckCircle, AlertCircle, X, Check } from "lucide-react";
import { z } from "zod";
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] shadow-2xl shadow-indigo-500/10 p-8 sm:p-10">
          ...
          {isSuccess ? (
            <div className="flex flex-col items-center justify-center text-center py-10 space-y-6 animate-in fade-in zoom-in duration-500">
              ...
              <div>
                <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">All Set!</h2>
                <p className="text-slate-500 mt-2 font-medium">Your password has been secured.<br/>Redirecting to login...</p>
              </div>
            ...
            <div>
              <div className="mb-8 text-center">
                ...
                <h1 className="text-3xl font-black text-slate-900 dark:text-white mb-3 tracking-tight">Secure Account</h1>
                <p className="text-slate-500 font-medium text-sm px-4">Create a strong password to verify and secure your identity.</p>
              </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
                        {[
                          ...
                          <div key={idx} className={`flex items-center gap-2 text-[11px] font-bold transition-colors ${check.pass ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400 dark:text-slate-500"}`}>
                            {check.pass ? <CheckCircle size={14} /> : <div className="w-[14px] h-[14px] rounded-full border border-slate-300 dark:border-slate-700 flex items-center justify-center" />}
                            {check.label}
                          </div>
                        ))}
                      </div>
