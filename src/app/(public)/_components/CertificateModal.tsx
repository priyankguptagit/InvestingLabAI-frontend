...
export default function CertificateModal({
  ...
  const handleValidate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await certificateApi.validateCertificate(
        certificateCode,
        userName,
      );
      if (res.success) {
        ...
        setError(
          res.message || "Validation failed. Please check your details.",
        );
      }
    } catch (err: any) {
      setError(
          ...
          {!validatedData ? (
            <form onSubmit={handleValidate} className="space-y-6 relative z-10">
              ...
              {error && (
                <Alert
                  variant="destructive"
                  className="bg-rose-500/10 border-rose-500/30 text-rose-400 rounded-xl animate-in fade-in zoom-in-95 duration-300"
                >
                  <AlertCircle className="h-4 w-4" />
                  <AlertTitle className="font-bold text-xs uppercase tracking-wider mb-1">
                    Validation Error
                  </AlertTitle>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 relative z-10">
              <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-4 flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-emerald-500/70 text-xs font-medium">
                    Authenticity confirmed by InvestingLab AI
                  </p>
                </div>
              </div>
              ...
              <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 space-y-4 relative overflow-hidden group">
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <Badge
                      variant="outline"
                      className="bg-emerald-500/10 text-emerald-400 border-emerald-500/30 font-black text-[9px] uppercase tracking-widest px-2 py-0"
                    >
                      Issued
                    </Badge>
                  </div>
                ...
                <div className="pt-4 border-t border-slate-800 mt-4 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs font-mono font-bold text-slate-400 mt-0.5">
                      {validatedData.certificateNumber}
                    </p>
                  </div>
