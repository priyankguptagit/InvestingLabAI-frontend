...
const TermsPopup = () => {
  ...
  const handleDecline = () => {
    alert(
      "You must accept the terms and conditions to proceed to the website.",
    );
  };
          ...
          >
            ...
            <div className="p-8 pb-4 text-center">
              <div className="flex items-center justify-center gap-3 mb-6">
                ...
                <span className="text-sm md:text-base font-bold text-white uppercase tracking-[0.3em]">
                  InvestingLab AI
                </span>
              </div>
              ...
              <p className="text-slate-400 text-sm md:text-base max-w-md mx-auto leading-relaxed">
                Please review these terms carefully before proceeding. Our
                relationship is built on transparency and professional
                integrity.
              </p>
            </div>
              ...
              <div className="mt-6 p-6 rounded-3xl bg-white/5 border border-white/10 shadow-lg">
                ...
                <div className="space-y-4 text-[13px] text-slate-400 leading-relaxed">
                  <p>
                    ...
                    Welcome to InvestingLab AI. By accessing or using our
                    services, you signify that you have read, understood, and
                  ...
                  <p>
                    ...
                    All quantitative models, proprietary scoring systems, and
                    editorial content displayed on the Praedico platform are the
                    exclusive intellectual property of InvestingLab AI.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-8 bg-[#020617]/80 border-t border-white/5 backdrop-blur-md">
              <label className="flex items-start gap-4 cursor-pointer group mb-8">
                <div className="relative mt-0.5">
                  ...
                  />
                  <div
                    className={`h-6 w-6 rounded-lg border-2 transition-all flex items-center justify-center shrink-0 ${agreed ? "bg-indigo-600 border-indigo-600 shadow-[0_0_15px_rgba(79,70,229,0.4)]" : "bg-transparent border-white/20 group-hover:border-white/40"}`}
                  >
                    {agreed && (
                ...
                <span className="text-sm text-slate-300 group-hover:text-white transition-colors leading-tight">
                  ...
                  <span className="text-indigo-400 underline underline-offset-4 decoration-indigo-400/30"></span>{" "}
...
const TermCard = ({ icon, title, points, color }: TermCardProps) => {
  ...
  return (
    <div
      className={`p-5 rounded-3xl border transition-all duration-500 hover:border-white/20 group/card ${colorMap[color]}`}
    >
      <div className="flex items-center gap-3 mb-4">
        <div className="p-2.5 rounded-[12px] bg-white/10 group-hover/card:scale-110 transition-transform">
          {icon}
        </div>
        <h4 className="font-bold text-[12px] tracking-[0.05em] uppercase text-white/90">
          {title}
        </h4>
      </div>
      <ul className="space-y-2.5">
        {points.map((p, i) => (
          <li
            key={i}
            className="flex items-start gap-2.5 text-[12px] text-slate-400 leading-snug group-hover/card:text-slate-300 transition-colors"
          >
