...
export const TradingPanel: React.FC<TradingPanelProps> = ({
  ...
  const canSell =
    tradeType === "SELL" && userHolding && quantity <= userHolding.quantity;
  ...
  const handleExecuteTrade = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setTimeout(
          () => setSuccessState({ show: false, message: "", qty: 0 }),
          3500,
        );
      }
    } catch (err: any) {
    ...
    <div className="relative w-full h-full flex flex-col min-h-[500px]">
      <AnimatePresence>
        {successState.show && (
          ...
          >
            ...
            <h2
              className={cn("text-3xl font-black mb-2 text-center", textStrong)}
            >
              Order Filled!
            </h2>
            <p className={cn("text-center text-lg font-medium", textMuted)}>
              Successfully {tradeType === "BUY" ? "purchased" : "sold"}{" "}
              <span className={cn("font-bold", textBase)}>
                {successState.qty} shares
              </span>{" "}
              of {symbol}.
            </p>
          </motion.div>
      ...
      {currentPlan === "Free" ? (
        <div className="flex-1 flex flex-col items-center justify-center p-8 rounded-2xl border border-red-500/20 bg-red-500/5 text-center">
          ...
          <h4 className="text-xl font-black text-red-400 mb-2">
            Paper Trading Locked
          </h4>
          <p className={cn("text-sm", textMuted)}>
            You need a Silver, Gold, or Diamond plan to access the live trading
            terminal.
          </p>
        </div>
      ) : (
        <div className="flex-1 flex flex-col space-y-6">
          ...
          <div
            className={cn(
              "relative flex p-1 rounded-xl bg-slate-900/10",
              border,
          ...
          >
            ...
            <motion.div
              ...
              animate={{
                left: tradeType === "BUY" ? "4px" : "calc(50% + 4px)",
              }}
          ...
          <div className="flex flex-col items-center justify-center py-4">
            ...
            <div className="flex items-center gap-6">
              <button
                ...
                className={cn(
                  "w-12 h-12 flex items-center justify-center rounded-full border transition-colors hover:bg-white/5 active:scale-95",
                  border,
                  textMuted,
                )}
              >
              ...
              <div className="relative group">
                ...
                <div
                  className={cn(
                    "absolute -bottom-2 left-0 right-0 h-0.5 scale-x-0 group-focus-within:scale-x-100 transition-transform origin-center",
          ...
          >
            ...
            {orderType === "LIMIT" && (
              <div className="flex justify-between items-center pt-2 border-t border-white/5">
                ...
                <input
                  ...
                  className={cn(
                    "w-28 px-3 py-1.5 text-right text-sm font-bold border rounded-lg focus:outline-none focus:ring-1 focus:ring-primary bg-transparent",
                    border,
                    textStrong,
                  )}
                />
            ...
            <div className="flex justify-between items-center pt-2 border-t border-white/5">
              <span className={cn("text-sm font-medium", textMuted)}>
                Available {tradeType === "BUY" ? "Funds" : "Shares"}
              </span>
              <span
                className={cn(
                  "text-sm font-bold",
          ...
          <div className="mt-auto pt-4 flex flex-col gap-3">
            <div className="flex justify-between items-end">
              ...
              <span
                className={cn("text-3xl font-black tracking-tight", textStrong)}
              >
                ₹
